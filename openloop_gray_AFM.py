...
def get_road_focused_feature_loss(feat_adv, feat_clean, road_ratio=0.45):
   ...
   # === 修改这里 ===
    ...
    # 检查全局配置 (需要在函数内能访问到，或者传入参数)
    # 简单的做法是直接访问全局 ATTACK_PARAMS
    if ATTACK_PARAMS.get("use_road_focus", True): 
...
class MFAttackerSiT:
    ...
    def run_attack(self, image_path, save_dir, fid_clean_dir, fid_adv_dir, file_prefix=""):
            ...
            raw_img_pil = Image.open(image_path).convert('RGB').resize((256,256))
            img_t = (torch.from_numpy(np.array(raw_img_pil)).permute(2,0,1).float().unsqueeze(0).to(self.device) / 127.5 - 1.0)
            ...
            with torch.no_grad():
                ...
                # [更新] 使用包含上下文的 prepare_simlingo_input
            ...
            with torch.no_grad():
                ...
                recon_img_448 = F.interpolate(recon_img, size=(448, 448), mode='bilinear', align_corners=False)
                img_f_448 = F.interpolate(img_f, size=(448, 448), mode='bilinear', align_corners=False)
                self.save_unified_visualization((recon_img_448+1)/2, (img_f_448+1)/2, r_clean, r_f, gt_route, tp1, s_clean, s_f, save_dir, img_name)
    ...
    def save_unified_visualization(self, recon_01, adv_01, rc, ra, gt, tp, sc, sa, save_dir, name):
        try:
            ...
            # 0. 保存干净图像 (SVG 格式，嵌入位图)
            clean_path = os.path.join(clean_img_dir, f"{name}_clean.svg")
            ...
            # 1. 保存攻击后图像 (SVG 格式)
            adv_path = os.path.join(adv_img_dir, f"{name}_adv.svg")
            ...
            # 2. 保存噪声图像 (SVG 格式)
            noise_path = os.path.join(noise_img_dir, f"{name}_noise.svg")
            ...
            # 3. 保存轨迹图像 (SVG 矢量格式)
            traj_path = os.path.join(traj_img_dir, f"{name}_traj.svg")
            fig_traj = plt.figure(figsize=(8, 8))
            ax = fig_traj.add_subplot(111)
            c_pts = rc[0].cpu().float().numpy(); a_pts = ra[0].cpu().float().numpy()
            if gt is not None: ax.plot(gt[:, 1], gt[:, 0], 'g-', alpha=0.3, linewidth=5, label='Ground Truth')
...
def main():
    ...
    # === 新增消融实验参数 ===
    parser.add_argument("--ablation_name", type=str, default="full", help="实验名称")
    parser.add_argument("--w_feature", type=float, default=3.0, help="特征MSE权重")
    parser.add_argument("--w_cosine", type=float, default=1.5, help="余弦相似度权重")
    parser.add_argument("--w_attn_focus", type=float, default=4.5, help="注意力权重")
    parser.add_argument("--road_focus", type=str, default="true", choices=["true", "false"], help="是否开启道路区域加权")
   ...
   # 修正后: 必须与 parser 定义一致 (.w_attn_focus)
    ATTACK_PARAMS["w_feature"] = args.w_feature
    ATTACK_PARAMS["w_cosine"] = args.w_cosine
    ATTACK_PARAMS["w_attn_focus"] = args.w_attn_focus  # 修正: args.w_attn -> args.w_attn_focus
    ATTACK_PARAMS["use_road_focus"] = (args.road_focus == "true")
    
    # 错误原代码: ATTACK_PARAMS["experiment_name"] = args.exp_name
    # 修正后: 必须与 parser 定义一致 (.ablation_name)
    ATTACK_PARAMS["experiment_name"] = args.ablation_name 
    
    print(f"\n>>> Running Experiment: {args.ablation_name}") # 修正: args.exp_name -> args.ablation_name
    print(f">>> Params: Feat={args.w_feature}, Cos={args.w_cosine}, Attn={args.w_attn_focus}, RoadFocus={args.road_focus}\n")
    # === 【修改结束】 ===
