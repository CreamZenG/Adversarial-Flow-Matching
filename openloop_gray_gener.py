...
class MFAttackerSiT:
    ...
    def run_attack(self, image_path, save_dir, fid_clean_dir, fid_adv_dir, file_prefix=""):
            ...
            orig_W, orig_H = raw_img_pil.size  # 记录原始宽和高 <--- 关键
            
            # 为了 VAE 攻击，暂时 resize 到 256x256
            input_pil = raw_img_pil.resize((256, 256), Image.BILINEAR)
           ...
           # === 3. 最终生成与完整指标计算 ===
            with torch.no_grad():
                ...
                # ================= [关键修改] 尺寸恢复逻辑 =================
                # 为了适应 TransFuser，必须恢复到原始图片的尺寸 (例如 160x704 或 900x1600)
                # 重新读取原图尺寸 (为了安全起见，直接在这里读取，无需修改函数开头)
                with Image.open(image_path) as tmp_img:
                    orig_W, orig_H = tmp_img.size
                
                # 将对抗样本 (img_f: 256x256) 插值回原始尺寸 (orig_H, orig_W)
                img_f_orig = F.interpolate(img_f, size=(orig_H, orig_W), mode='bilinear', align_corners=False)
                recon_img_orig = F.interpolate(recon_img, size=(orig_H, orig_W), mode='bilinear', align_corners=False)
                # =========================================================
                
                # 为了计算 SimLingo 的指标，依然生成 512 版本 (保持原有逻辑兼容性)
                ...
                # 注意：这里依然输入 448x448 给 SimLingo，但这只是为了看 Log，保存的是 img_f_orig
                ...
                # 3. 保存文件
                save_safe_png(recon_img_orig, os.path.join(fid_clean_dir, save_name))
                save_safe_png(img_f_orig, os.path.join(fid_adv_dir, save_name))
                save_safe_png(img_f_orig, os.path.join(save_dir, save_name))
                # =========================================================
                # =========================================================
