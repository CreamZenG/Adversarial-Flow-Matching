...
def find_valid_datasets(base_path):
    ...
    这样可以确保获取到的是 'Town05_Short' 而不是 'test_turn'。
    """
    valid_routes = []
    ...
    for root, dirs, files in os.walk(base_path):
        # 只有当当前目录包含 'measurements' 时，才认为它是一个有效的 Route 目录
        if 'measurements' in dirs:
            valid_routes.append(root)
            
    if not valid_routes:
        # 如果还没找到，检查 base_path 本身是否就是
...
class TransFuserMFAAttacker:
    ...
    def run_diff_attack(self, rgb_raw, batch_data):
        ...
        bs = rgb_raw.shape[0]
        img_256 = F.interpolate(img_01, size=(256, 256), mode='bilinear')
        img_norm_256 = (img_256 * 2.0) - 1.0 
        ...
        with torch.no_grad():
            ...
            z_mid = self.invert_latents(latents_orig, class_labels, ATTACK_PARAMS['t_limit'], ATTACK_PARAMS['sample_steps'])
        ...
        for _ in range(ATTACK_PARAMS['iterations']):
            ...
            z_final = self.differentiable_flow_sampler(z_start, class_labels, ATTACK_PARAMS['t_limit'], delta_u_dict=delta_u_seq, num_steps=ATTACK_PARAMS['sample_steps'])
            
            if torch.isnan(z_final).any(): break

            img_adv_norm = self.vae.decode(z_final / 0.18215).sample
            img_adv_norm = torch.clamp(img_adv_norm, -1, 1) 
        ...
        with torch.no_grad():
            final_z = self.differentiable_flow_sampler(z_mid + delta_z, class_labels, ATTACK_PARAMS['t_limit'], delta_u_dict=delta_u_seq, num_steps=ATTACK_PARAMS['sample_steps'])
    ...
    def run_patch_attack(self, rgb_raw, batch_data):
        ...
        for i in range(3):
            ...
            delta_z = nn.Parameter(torch.randn_like(z_mid) * 1e-4)
            delta_u_seq = nn.ParameterDict({
                str(k): nn.Parameter(torch.zeros_like(z_mid)) for k in range(ATTACK_PARAMS['sample_steps'])
            })
...
def main():
    ...
    print("=== TransFuser MFA Attack (SiT Version) - Continuous Route Mode ===")
    ...
    # [Fix] 使用新的扫描逻辑，确保获取到的是路线文件夹而非父文件夹
        ...
        try:
            ...
            for batch_idx, data in enumerate(tqdm(loader, desc=f"Attack {ds_name}")):
                ...
                try:
                    ...
                    # === 保存逻辑 ===
                    # 命名格式：RouteName_0000.png (例如 Town05_Short_0000.png)
