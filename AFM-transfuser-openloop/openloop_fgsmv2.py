...
ATTACK_PARAMS = {
    ...
    # === 特征攻击 Loss 权重 (MFA Style) ===
    "w_feature": 3.0,       # 特征距离权重 (MSE)
    "w_cosine": 1.5,        # 方向差异权重 (Cosine Similarity)
    "w_attn": 6.0,          # 注意力加权特征权重 (新增)
    "attn_temp": 6.0        # 注意力温度参数 (新增)
}

# ================= Monkey Patching 函数 =================
# 用于动态替换 TransFuser 中 SelfAttention 的 forward 方法，以捕获 Attention Map
...
def get_transfuser_attention_loss(curr_img_feat, curr_lidar_feat, target_img_feat, target_lidar_feat, clean_attn_map, temperature):
    """
    计算基于 Attention 权重的特征 Loss。
    由于 TransFuser 没有 CLS Token，我们计算每个 Token 的全局重要性（平均被关注度）作为权重。
    """
    if clean_attn_map is None:
        return 0.0

    b, c, h_img, w_img = curr_img_feat.shape
    _, _, h_lid, w_lid = curr_lidar_feat.shape
    
    # 1. 展平并拼接 Image 和 LiDAR 特征 [B, N, C]
    # Image: [B, C, H, W] -> [B, C, N_img] -> [B, N_img, C]
    ...
    # 2. 计算重要性权重 (Importance Weights)
    # clean_attn_map Shape: [B, Heads, N, N] (Query, Key)
    # 我们对 Heads 和 Query 维度求平均，得到每个 Key Token 被关注的平均强度
    # Shape: [B, N]
    ...
    # 对 N 维度加权求和，然后对 Batch 和 Channel 求平均
...
class TransFuserAttacker:
    def __init__(self, model, device):
        ...
        # [关键] Monkey Patching: 替换 Transformer1 中所有 Block 的 SelfAttention forward
        # 以便获取 Attention Map 用于 Loss 计算
        if hasattr(self.model, '_model') and hasattr(self.model._model, 'transformer1'):
            ...
            for i, block in enumerate(tf_blocks):
                # 动态替换 forward 方法，绑定到特定实例
                block.attn.forward = types.MethodType(list_capturing_forward, block.attn)
            print(f">> Monkey-Patch applied to {len(tf_blocks)} Attention layers in Transformer1 for MFA Attack.")
        else:
             print("Warning: Could not find Transformer1 blocks for Monkey Patching. Attention loss will be disabled.")
    ...
    def run_attack(self, rgb_raw, lidar, target_point, target_point_image, ego_vel, bev_points=None, cam_points=None, num_points=None):
        """
        FGSM Attack with Full MFA Loss (MSE + Cosine + Attention)
        """
        epsilon_255 = ATTACK_PARAMS['epsilon'] * 255.0
        ...
        with torch.no_grad():
            ...
            # [MFA] 获取 Clean Attention Map (取最后一层)
            clean_attn_map = None
            try:
                ...
                pass # Monkey patch might have failed or model structure differs

        # 2. Gradient Calculation
        rgb_input = rgb_raw.clone().detach()
        noise = torch.zeros_like(rgb_input).uniform_(-1e-6, 1e-6) # 避免零梯度
        rgb_input = rgb_input + noise
        rgb_input.requires_grad = True

        self._hook_features = {} 
        self.model(rgb_input, lidar, dummy_wp, target_point, target_point_image, ego_vel, 
                   dummy_bev, dummy_label, dummy_depth, dummy_semantic, num_points=num_points)
        ...
        # 3. Attention Weighted component (新增)
        loss_attn = 0.0
        ...
        # 优化目标：Maximize MSE & Attn_Weighted_MSE, Minimize Cosine
        # Minimize Total_Loss
        total_loss = - ATTACK_PARAMS['w_feature'] * loss_mse \
        ...
        grad = rgb_input.grad.data
        
        # 3. FGSM Update
        if grad is not None:
            rgb_adv = rgb_input - epsilon_255 * grad.sign()
            rgb_adv = torch.clamp(rgb_adv, 0, 255).detach()
        else:
            rgb_adv = rgb_raw.clone()
...
def save_visualization_1x4(rgb_clean, rgb_adv, pred_clean, pred_adv, gt_wp, target_point, save_path, frame_id, metrics_str):
    ...
    ax2.imshow(img_adv_np); ax2.set_title("Adv (FGSM-MFA)"); ax2.axis('off')
...
def main():
    ...
    parser.add_argument('--output_dir', type=str, default='fgsm_mfa_results')
    ...
    print(f"=== TransFuser FGSM-MFA Attack ===")
    ...
    if len(global_stats['shift']) > 0:
        ...
        print("\n" + "="*60)
        print("MACRO AVERAGE METRICS (FGSM-MFA):")
