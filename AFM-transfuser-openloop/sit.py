...
class SiTBlock(nn.Module):
    ...
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, **block_kwargs):
        ...
        # [修改 1] 删除了 qk_norm 参数，兼容旧版 timm
        # [修改 2] 删除了 block_kwargs["qk_norm"] 的字典访问，防止 KeyError
        self.attn = Attention(
            hidden_size, num_heads=num_heads, qkv_bias=True
        )
        
        # 注意: 旧版 timm 的 Attention 通常也不支持 fused_attn，这里保留判断但通常不会生效
        if "fused_attn" in block_kwargs.keys() and hasattr(self.attn, 'fused_attn'):
...
class SiT(nn.Module):
    ...
    def __init__(
        ...
        **block_kwargs # fused_attn
        ...
        # [修改 3] 删除了 bias=True，兼容旧版 timm.models.layers.PatchEmbed
        ...
        # [修改 4] 我们在这里拦截 qk_norm，防止它进入 block_kwargs 并导致兼容性问题
        # 即使外部传了 qk_norm，这里也不会报错，只是默默忽略
        block_kwargs.pop('qk_norm', None)
