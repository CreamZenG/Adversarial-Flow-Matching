...
class TransFuserMFAAttacker:
    ...
    def run_patch_attack(self, rgb_raw, batch_data):
        """
        分区攻击方法：将 704x160 图像分成三个摄像头区域
        分别编码到 latent 空间，然后联合优化，最后合并
        """
        bs = rgb_raw.shape[0]
        ...
        with torch.no_grad():
            for i in range(3):
                ...
                patch_latents_orig.append(latents_orig)
                patch_z_mids.append(z_mid)
        
        # === Step 3: 联合优化三个 patch ===
        delta_zs = [nn.Parameter(torch.randn_like(z) * 1e-4) for z in patch_z_mids]
        delta_u_seqs = [
            nn.ParameterDict({str(j): nn.Parameter(torch.zeros_like(patch_z_mids[i])) 
                             for j in range(ATTACK_PARAMS['sample_steps'])})
            for i in range(3)
        ]
        
        all_params = []
        for dz in delta_zs:
            all_params.append({'params': [dz], 'lr': ATTACK_PARAMS['lr_z']})
        for du_seq in delta_u_seqs:
            all_params.append({'params': du_seq.parameters(), 'lr': ATTACK_PARAMS['lr_u']})
        
        optimizer = optim.Adam(all_params)
        real_z_std = sum(lat.std() for lat in patch_latents_orig) / 3
        
        for iter_idx in range(ATTACK_PARAMS['iterations']):
            optimizer.zero_grad()
            ...
            for i in range(3):
                z_start = patch_z_mids[i] + delta_zs[i]
                z_final = self.differentiable_flow_sampler(
                    z_start, class_labels, ATTACK_PARAMS['t_limit'],
                    delta_u_dict=delta_u_seqs[i], num_steps=ATTACK_PARAMS['sample_steps']
                )
                ...
                adv_patch = F.interpolate(img_adv_01, size=patch_sizes[i], mode='bilinear')
                adv_patches.append(adv_patch)
            
            if len(adv_patches) != 3:
                break
            
            # B. 拼接成完整图像
            img_adv_full = torch.cat(adv_patches, dim=3)
            ...
            loss_road = self.get_road_focused_loss(feat_adv, feat_clean)
            loss_attn = self.get_attention_weighted_loss(feat_adv, feat_clean, clean_tf_attn)
            loss_anchor = sum(torch.abs(zf.std() - real_z_std) for zf in z_finals) / 3
            ...
            with torch.no_grad():
                for dz in delta_zs:
                    dz.data.clamp_(-ATTACK_PARAMS['epsilon_latent'], ATTACK_PARAMS['epsilon_latent'])
                for du_seq in delta_u_seqs:
                    for k in du_seq:
                        du_seq[k].data.clamp_(-ATTACK_PARAMS['epsilon_u'], ATTACK_PARAMS['epsilon_u'])
        ...
        with torch.no_grad():
            ...
            for i in range(3):
                final_z = self.differentiable_flow_sampler(
                    patch_z_mids[i] + delta_zs[i], class_labels, ATTACK_PARAMS['t_limit'],
                    delta_u_dict=delta_u_seqs[i], num_steps=ATTACK_PARAMS['sample_steps']
                )
                final_norm = self.vae.decode(final_z / 0.18215).sample
                final_01 = (torch.clamp(final_norm, -1, 1) / 2.0 + 0.5)
                final_patch = F.interpolate(final_01, size=patch_sizes[i], mode='bilinear')
