...
class DiffAttackController:
    ...
    def step(self):
        # 删除 self.cur_step += 1; self.layer_idx = 0
        pass
        
    def __call__(self, attn_probs, is_cross, layer_name):
        # 我们只控制 Self-Attention，Cross-Attention 直接返回
        if is_cross: return attn_probs
        
        # 使用唯一的 layer_name 作为 Key，保证绝对匹配
        key = layer_name
...
class TransFuserDiffAttacker:
    def __init__(self, transfuser_model, device):
        ...
        try:
            ...
            # 方案 B：如果没有 xformers，开启 attention slicing (速度稍慢，极大节省显存)
    ...
    def run_diff_attack(self, rgb_raw, batch_data):
        ...
        # === 【核心修复】初始化干净的 Controller 用于 Inversion ===
        # 必须在 DDIM Inversion 之前重置，避免上一个样本的状态残留
        controller = DiffAttackController()
        controller.mode = "off" # 设置为 off，使其在 Inversion 时透传 Attention，不干预
        ...
        # === Step 3: SD Attention Control (Start Recording) ===
        # 复用已注册的 Controller，切换为 store 模式，记录初始噪声下的 Attention Maps
        ...
        for _ in range(ATTACK_PARAMS['iterations']):
            ...
            # Step D: 输入 TransFuser 提取特征 (使用 Dummy Tensors)
            self.tf_extractor.clear()
            self.tf_model(
    ...
    def run_patch_attack(self, rgb_raw, batch_data):
        ...
        for patch_idx in range(3):
            ...
            patch_size = patch.shape[2:]  # (160, ~234)
            
            # Resize to 256x256 for SD VAE (节省显存)
...
def main():
    ...
    parser.add_argument('--output_dir', type=str, default='da_aligned_results')
    ...
    print("=== TransFuser DiffAttack (SimLingo Aligned) ===")
    ...
    if len(global_stats['shift']) > 0:
        ...
        print("\n" + "="*60)
        print("MACRO AVERAGE METRICS (SimLingo Aligned):")
