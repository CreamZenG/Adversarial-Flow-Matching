...
# ================= 1. FGSM 攻击配置 (适配 MFA 结构) =================
ATTACK_PARAMS = {
    # === FGSM 参数 ===
    "epsilon": 8/255,       # 扰动强度 (L_inf 约束)
    "alpha": 8/255,         # 单步步长 (FGSM 要求 alpha = epsilon)
    "steps": 1,             # 迭代次数 (FGSM 固定为 1)
...
class TransFuserFGSMAttacker:
    def __init__(self, transfuser_model, device):
        ...
        if ATTACK_PARAMS['steps'] != 1:
            print(f">>> Warning: FGSM requires steps=1. Resetting from {ATTACK_PARAMS['steps']} to 1.")
            ATTACK_PARAMS['steps'] = 1
        
        # FGSM 要求单步攻击强度等于 epsilon
        ATTACK_PARAMS['alpha'] = ATTACK_PARAMS['epsilon']
        print(f">>> FGSM Configured: Steps=1, Epsilon={ATTACK_PARAMS['epsilon']:.4f}, Alpha={ATTACK_PARAMS['alpha']:.4f}")
    ...
    def run_attack(self, rgb_raw, batch_data):
        """
        FGSM Attack (Single Step Gradient Ascent)
        ...
        # Objective: Maximize Feature Drift (MSE up, Cosine down)
...
def save_visualization_unified(rgb_clean, rgb_adv, pred_clean, pred_adv, gt_wp, target_point, save_path, drift, title_prefix=""):
    ...
    noise = np.clip(np.abs(img_adv - img_clean) * 15.0, 0, 1) # FGSM noise is small, amplify x15
    ...
    ax2.set_title("2. Adv (FGSM)", fontsize=14, fontweight='bold'); ax2.axis('off')
...
def main():
    ...
    parser.add_argument('--output_dir', type=str, default='fgsm_mfa_style_results')
    ...
    print("=== TransFuser FGSM Attack (MFA Style) ===")
    ...
    attacker = TransFuserFGSMAttacker(model, device)
    ...
    if len(global_stats['shift']) > 0:
        ...
        print("\n" + "="*60)
        print("MACRO AVERAGE METRICS (FGSM - MFA Style):")
