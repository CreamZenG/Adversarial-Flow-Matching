...
class SimLingoNCFAttacker:
    def __init__(self, config):
        ...
        # 【关键修复】：恢复 BFloat16 默认类型，与 FGSM 代码保持一致，适配模型权重
    ...
    def prepare_inputs(self, image_path):
        ...
        raw_pil = Image.open(image_path).convert('RGB').resize((448, 448))
        
        # 基础输入为 Float32 (用于攻击计算)，后续需要转 BFloat16 (用于模型推理)
        img_tensor = T.ToTensor()(raw_pil).unsqueeze(0).to(self.device).float()
        ...
        return img_tensor, meta, raw_pil, gt_route, speed, tp1
    ...
    def run(self, img_path, output_dir, fid_clean_dir, fid_adv_dir, file_prefix=""):
        ...
        img_clean, meta, pil_clean, gt_route, clean_speed, target_point = self.prepare_inputs(img_path)
        ...
        with torch.no_grad():
            ...
            save_safe_jpg(img_clean.cpu().float(), os.path.join(fid_clean_dir, save_name))
            save_safe_jpg(img_adv.cpu().float(), os.path.join(fid_adv_dir, save_name))
    ...
    def save_unified_visualization(self, recon_01, adv_01, route_clean, route_adv, 
        ...
        try:
            ...
            ax.set_title("Trajectory (NCF Attack)", fontsize=14, fontweight='bold')
            plt.tight_layout()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", type=str, default="ncf_whitebox_results") 
    ...
    for folder in existing_folders:
        if folder.startswith("NCF-Grad-"):
            try:
                idx = int(folder.split("-")[-1])
                if idx > max_id: max_id = idx
            except ValueError: continue
    
    new_folder_name = f"NCF-Grad-{max_id + 1}"
